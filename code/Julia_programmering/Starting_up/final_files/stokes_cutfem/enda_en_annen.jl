...
function stokes_solver(;n, u_exact, p_exact, f, g, ud, order, geometry, βu0, γu1, γu2, γp, βp0, nu, stabilize, δ, save = false, calc_condition)
    """
    ...
    a(u, v) = (∫(flux(ε(u)) ⊙ ε(v))dΩ + ∫(-((n_Γd ⋅ (flux(ε(u)))) ⋅ v) + (-(n_Γd ⋅ (flux(ε(v)))) ⋅ u)+(γ/h * u ⋅ v))dΓd )        # faktor 2 på den siste???
    b(v, p) = (∫(-1*(∇ ⋅ v*p))dΩ
              + ∫((n_Γd ⋅ v) * p)dΓd)#
 

    gu(u,v) = ( ∫( (β1*h)*jump(n_Fg⋅∇(u))⋅jump(n_Fg⋅∇(v)) )dFg 
          +  
             ∫( (β2*h^3)*jump_nn(u,n_Fg)⋅jump_nn(v,n_Fg) )dFg)
    ...
    l1(v) = ∫(f ⋅ v)dΩ
    l2(v) = ∫(-1* (n_Γd ⋅ (flux(ε(v)))) ⋅ ud + γ/h ⋅ v ⋅ ud )dΓd   # faktor 2 på den siste???
    l3(q) = ∫(n_Γd ⋅ ud *q)dΓd
