...
geometry = "heart"
βu0 = 8 * 10
βp0 = 6  # får ok resultater når denne er 1 også??
β_1 = 1
β_2 = 1
β_3 = 0.1
γ=1
...
function sigmund_solver(;n, u_exact, p_exact, f, g, ud, order, geometry, βu0, γu1, γu2, γp, βp0, nu, stabilize, δ, save = false, calc_condition = false)
      """
      ...
      γ = 5*nu0*order*(order+1)/h
      βu = βu0 *nu0/(h^2)                # disse er de samme som Sigmund bruker...
      βp = βp0*nu0/( h) 
      mu = nu0*γ/h
    ...
    gu(u,v) = (∫( 1/3*h^3* (jump(n_Fg ⋅ ∇(u))⋅ jump(n_Fg ⋅∇(v))))dFg       
               + ∫(1/20*h^5 *(jump_nn(u, n_Fg) ⋅ jump_nn(v, n_Fg)))dFg
)

    gp(p,q) = (∫( 1/3*h^5* (jump(n_Fg ⋅ ∇(p))*jump(n_Fg ⋅ ∇(q))))dFg)
...
#     title_str="Convergence of CutFEM"
# )
...
#     start = start,
#     title_str = "Sensitivity of CutFEM Stokes"
# )
