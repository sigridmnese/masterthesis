...
function append_convergence_row(n::Int; stabilize::Bool = true)
    # --- kall solver ---
    uh, u_exact, erru, ul2_norm, uh1_semi,
    ph, p_exact, errp, pl2_norm, ph1_semi,
...
function stokes_navierBC_CutFEM_3d(;n, u_exact, p_exact, f, g, ud, order, geometry, βu0, γu1, γu2, γp, βp0, nu, stabilize, δ, save = false, calc_condition = false)
    ...
    # %% Test STL geo
    geo_stl = STLGeometry("droplet.stl")
    #geo_stl = STLGeometry("droplet.stl"; scale=2.0, translation=Point(0.0, 0.0, -0.5))
...
harr     = df.h[[3, 1, 2]]
uarr_l2_stab  = df.u_l2[[3, 1, 2]]
uarr_h1_stab  = df.u_h1[[3, 1, 2]]
parr_l2_stab  = df.p_l2[[3, 1, 2]]
parr_h1_stab  = df.p_h1[[3, 1, 2]]
