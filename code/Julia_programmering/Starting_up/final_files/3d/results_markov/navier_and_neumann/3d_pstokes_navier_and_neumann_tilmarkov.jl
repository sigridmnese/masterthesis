...
# solver:
function pstokes_CutFEM_navierandneumann_3d(;n, u_exact, p_exact, f, g, ud, order, geometry, βu0, γu1, γu2, γp, βp0, nu, stabilize, δ, save = false, calc_condition = false)
    """    
     """
    ...
    if save
        writevtk(Ω, "3d_navier_and_neumann $n.vtu", cellfields=["u_ex" => u_exact, "uh"=>uh, "erru"=> erru, "p_ex" => p_exact, "ph"=>ph, "errp"=> errp, "nablau" => ∇(u_exact), "viskositet" => viskositet∘ε(u_exact)]) #, "erru" => erru]) 
    end
...
n = 64
stabilize = true
solver = pstokes_CutFEM_navierandneumann_3d
δ = 0 
save = true
calc_condition = false
order = 2
...
# CSV.write("3d_convergence_results_navier_and_neumann.csv", df_convergence)
